...
void RunManager::initLogic()
{
	...
	mLogic.transition(TimerEvent::EXPECT_NEW,
			...
			[this](void)
			{
				auto t = data::Location::Type::HAND_OVER;
				auto l = new data::Location(t);
				data::Data_ptr p(l);
				this->sendMessage(Location::MASTER, runMessage_t::ITEM_DISAPPEARED, std::move(p));
			});
	//LB_START_1
	...
	mLogic.transition(run::HalEvent::LB_HS,
			...
			[this](void)
			{
				this->sendMessage(Location::MASTER, runMessage_t::ITEM_APPEARED, MXT_SHARE(data::Location::Type::LB_HEIGHTSENSOR));
			});
	...
	mLogic.transition(run::HalEvent::LB_SWITCH,
			...
			[this](void)
			{
				this->sendMessage(Location::MASTER, runMessage_t::ITEM_APPEARED, MXT_SHARE(data::Location::Type::LB_SWITCH));
			});
	...
	mLogic.transition(run::HalEvent::LB_RAMP,
			...
			[this](void)
			{
				this->sendMessage(Location::MASTER, runMessage_t::ITEM_APPEARED, MXT_SHARE(data::Location::Type::LB_RAMP));
			});
	//TIMER_RAMP
	mLogic.transition(TimerEvent::RAMP,
			...
			[this](void)
			{
				this->sendMessage(Location::MASTER, runMessage_t::RAMP_FULL, MXT_SHARE(data::Location::Type::LB_RAMP));
			});
	//TIMER_RAMP
